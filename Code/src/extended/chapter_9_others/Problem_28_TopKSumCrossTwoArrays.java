...
public class Problem_28_TopKSumCrossTwoArrays {
    ...
    public static int[] topKSum(int[] a1, int[] a2, int topK) {
        ...
        //NOTE:注意,heapSize先参与方法的执行,该方法执行结束后在进行++;
    ...
    // For test, this method is inefficient but absolutely right,居然用到了全排列...
    ...
    public static void main(String[] args) {
        ...
        System.out.println("-------------------------");
        start = System.currentTimeMillis();
        int[] myRes = myTopKSum(arr1, arr2, k);
        end = System.currentTimeMillis();
        System.out.println(end - start + " ms");
    ...
    static int[] myTopKSum(int[] arr1, int[] arr2, int k) {
        int[] res = new int[k];
        int sum = 0;
        PriorityQueue<HeapNode> queue = new PriorityQueue(new Comparator() {
            @Override
            public int compare(Object o1, Object o2) {
                return ((HeapNode) o2).value - ((HeapNode) o1).value;
            }
        });

        int i = arr1.length - 1;
        int j = arr2.length - 1;
        queue.add(new HeapNode(i, j, arr1[i] + arr2[j]));
        while (sum < k) {
            ...
            j = node.row;
            if (i > 0) {
                queue.add(new HeapNode(i - 1, j, arr1[i - 1] + arr2[j]));
            }
            if (j > 0) {
                queue.add(new HeapNode(i, j - 1, arr1[i] + arr2[j - 1]));
            }
        }
        return res;
    }
